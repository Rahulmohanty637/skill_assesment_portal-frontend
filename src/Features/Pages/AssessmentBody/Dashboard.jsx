...
const SideBar = ({ children }) => {
  ...
  const [selectedComponent, setSelectedComponent] = useState("Assessment Schedule");
  ...
  return (
    <div className="flex h-screen">
      <div className={`lg:flex flex-col bg-gray-800 text-white transition-width duration-300 ${isOpen ? "w-64" : "w-16"} lg:w-64`}>
        <div className="p-4 text-lg font-semibold border-b border-gray-700 flex justify-between items-center">
          <span className={`${isOpen ? "block" : "hidden"} lg:block`}>Skill Assessment Dashboard</span>
            ...
            {routes.map((route, index) => (
              route.subRoutes ? (
                ...
                >
                  ...
                  <span className={`${isOpen ? "block" : "hidden"} lg:block`}>{route.name}</span>
                </div>
              )
            ))}
          </div>
